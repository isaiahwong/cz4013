...
class ErrorMsg:
    ...
    def unmarshall(cls,data):
        ...
        lenbodybytes = struct.unpack_from('<H', data[lenerrbytes+2:4+lenerrbytes])[0]
...
class message:
    ...
    def unmarshall(cls, data):
        ...
        lenbody =  struct.unpack_from('<H', data[4+lenrpcbytes+lenquery:6+lenrpcbytes+lenquery])[0]
        body = bytearray(data[6+lenrpcbytes+lenquery:6+lenrpcbytes+lenquery+lenbody])
        #check if there is an error in the first place
        if data[6+lenrpcbytes+lenquery+lenbody:]:
            lenerror = struct.unpack_from('<H', data[6+lenrpcbytes+lenquery+lenbody:8+lenrpcbytes+lenquery+lenbody])[0]
