...
class message:
    ...
    def marhsall(self):
        ...
        if self.error:
            ...
            return struct.pack('<H', len(rpcbytes)) + struct.pack('<' + str(len(rpcbytes))+'s', rpcbytes) + struct.pack('<H', len(querybytes))+ struct.pack('<' + str(len(querybytes))+'s', querybytes)+self.body+errbytes
        else:
            return struct.pack('<H', len(rpcbytes))+struct.pack('<' + str(len(rpcbytes))+'s', rpcbytes)+struct.pack('<H', len(querybytes))+struct.pack('<' + str(len(querybytes))+'s', querybytes)+self.body
